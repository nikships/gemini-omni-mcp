...
- Use `generation_config={"video_config":{"task": ...}}` for `text_to_video`, `image_to_video`, `reference_to_video`, and `extend`. Omit it for `edit` calls that use `previous_interaction_id` or uploaded videos.
